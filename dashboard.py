...
class ModernDashboard(QMainWindow):
    def __init__(self):
        ...
        # 🟢 เซ็ตขนาดเริ่มต้นแบบไม่ล็อก เพื่อให้สามารถขยายได้
        ...
        # ใช้ QTimer เพื่อให้ทำงานแทน Thread ทั่วไป (ปลอดภัยกับ GUI)
    ...
    def init_ui(self):
        ...
        # 🟢 ลบ Outer ScrollArea ออก เพื่อให้โครงสร้าง Layout ปรับหดตาม Window ได้อิสระ
        main_page_layout = QHBoxLayout(self.page_normal)
        main_page_layout.setSpacing(15)
        main_page_layout.setContentsMargins(15, 15, 15, 15)
        ...
        main_page_layout.addLayout(self.left_layout, stretch=6)
        main_page_layout.addLayout(right_layout, stretch=4)
        ...
        cam_top = QHBoxLayout()
        cam_title = QLabel("MAIN CAMERA")
        cam_title.setObjectName("CardTitle")
        
        self.combo_res = QComboBox()
        self.combo_res.addItems(["360p (640x360)", "720p (1280x720)", "1080p (1920x1080)"])
        self.combo_res.setCurrentIndex(1) 
        self.combo_res.setStyleSheet(f"QComboBox {{ background-color: #2C2C2E; color: {TEXT_WHITE}; border-radius: 4px; padding: 5px; font-size: 12px; border: 1px solid #333; }}")
        
        self.btn_stream = QPushButton("▶ START")
        self.btn_stream.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_GREEN}; color: #121212; padding: 5px 15px; border-radius: 4px; font-size: 11px; }} QPushButton:hover {{ background-color: #3BEA55; }}")
        self.btn_stream.clicked.connect(self.toggle_stream)
        
        self.btn_snap = QPushButton("📸 CAPTURE")
        self.btn_snap.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_BLUE}; padding: 5px 15px; border-radius: 4px; font-size: 11px; }} QPushButton:hover {{ background-color: #47A1FF; }}")
        self.btn_snap.clicked.connect(self.capture_image)
        
        self.btn_rec = QPushButton("🎥 RECORD")
        self.btn_rec.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_ORANGE}; padding: 5px 15px; border-radius: 4px; font-size: 11px; }} QPushButton:hover {{ background-color: #FFB340; }}")
        self.btn_rec.clicked.connect(self.toggle_video_recording)

        self.btn_full = QPushButton("⛶ FULL")
        self.btn_full.setStyleSheet(f"QPushButton {{ background-color: #3A3A3C; color: white; padding: 5px 15px; border-radius: 4px; font-size: 11px; }} QPushButton:hover {{ background-color: #505052; }}")
        ...
        cam_top.addWidget(self.btn_full)
        ...
        # 🟢 เซ็ตขนาดขั้นต่ำให้เป็น 1 เพื่อให้เฟรมย่อตัวได้อิสระสุดๆ โดยให้ Layout รับภาระยืด-หดแทน
        ...
        self.btn_restart.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_ORANGE}; color: white; }} QPushButton:hover {{ background-color: #FFB340; }}")
        self.btn_restart.clicked.connect(self.restart_program)
        
        self.btn_stop = QPushButton("EMERGENCY STOP")
        self.btn_stop.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_RED}; color: white; }} QPushButton:hover {{ background-color: #FF6961; }}")
        ...
        self.btn_test_mode.setStyleSheet(f"QPushButton {{ background-color: #3A3A3C; color: {TEXT_DIM}; padding: 5px 15px; border-radius: 11px; font-size: 11px; font-weight: bold; }} QPushButton:checked {{ background-color: {ACCENT_ORANGE}; color: #121212; }} QPushButton:hover {{ background-color: #646468; }}")
        ...
        self.hull_card = self.create_card()
        hull_layout = QVBoxLayout(self.hull_card)
        hull_top = QHBoxLayout()
        hull_title = QLabel("CTL BATT"); hull_title.setObjectName("CardTitle")
        ...
        self.row2_layout.addWidget(self.hull_card, stretch=3)

        self.thrust_batt_card = self.create_card()
        thrust_layout = QVBoxLayout(self.thrust_batt_card)
        thrust_top = QHBoxLayout()
        thrust_title = QLabel("THR BATT"); thrust_title.setObjectName("CardTitle")
        ...
        self.row2_layout.addWidget(self.thrust_batt_card, stretch=3)
        ...
        sliders_grid.setVerticalSpacing(5) # 🟢 ลดช่องว่าง
        self.sliders = []
        ...
        for i in range(8):
            ...
            slider.setStyleSheet(f"QSlider::groove:horizontal {{ border-radius: 4px; height: 6px; background: #2C2C2E; }} QSlider::handle:horizontal {{ background: {ACCENT_GREEN}; width: 25px; height: 18px; margin: -4px 0; border-radius: 8px; }}")
            lbl_val = QLabel("1500")
            lbl_val.setStyleSheet("background-color: #2C2C2E; color: #F5F5F7; padding: 4px 6px; border-radius: 4px; font-weight: bold; font-family: monospace;")
            lbl_val.setFixedWidth(50)
            lbl_val.setAlignment(Qt.AlignCenter)
            
            btn_reset = QPushButton("⟲")
            btn_reset.setStyleSheet("QPushButton { background-color: #3A3A3C; border-radius: 12px; font-size: 14px; padding: 4px; } QPushButton:hover { background-color: #505052; }")
        ...
        self.tabs.setStyleSheet(f"QTabWidget::pane {{ border: 1px solid #333; border-radius: 4px; background: {BG_CARD}; top: -1px; }} QTabBar::tab {{ background: #2C2C2E; color: #8E8E93; padding: 8px 10px; border-top-left-radius: 4px; border-top-right-radius: 4px; margin-right: 2px; font-weight: bold; font-size: 12px; }} QTabBar::tab:selected {{ background: {ACCENT_BLUE}; color: white; }}")
        ...
        self.btn_manual.setStyleSheet(f"QPushButton {{ background-color: #3A3A3C; padding: 6px 10px; border-radius: 12px; font-size: 12px; font-weight: bold; }} QPushButton:hover {{ background-color: #646468; }}")
        self.btn_manual.clicked.connect(self.toggle_manual_control)
        mode_row = create_row("MODE", [self.lbl_joy_status, self.btn_manual])
        
        self.btn_ramp = QPushButton("RAMP: ON")
        self.btn_ramp.setCheckable(True)
        self.btn_ramp.setChecked(True)
        self.btn_ramp.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_ORANGE}; color: black; padding: 6px 10px; border-radius: 12px; font-size: 12px; font-weight: bold; }} QPushButton:hover {{ background-color: #ffad33; }}")
        self.btn_ramp.clicked.connect(self.toggle_ramp_mode)
        
        self.lbl_ramp_val = QLabel("0.25")
        self.lbl_ramp_val.setStyleSheet("background-color: #2C2C2E; color: #F5F5F7; border-radius: 4px; font-weight: bold; font-family: monospace; font-size: 12px;")
        ...
        self.slider_ramp.setStyleSheet(f"QSlider::groove:horizontal {{ border-radius: 3px; height: 6px; background: #2C2C2E; }} QSlider::handle:horizontal {{ background: {ACCENT_ORANGE}; width: 16px; height: 16px; margin: -3px 0; border-radius: 5px; }}")
        self.slider_ramp.valueChanged.connect(lambda v: self.lbl_ramp_val.setText(f"{v/100.0:.2f}"))
        profile_row = create_row("PROFILE", [self.btn_ramp, self.slider_ramp, self.lbl_ramp_val])

        btn_all_min = QPushButton("MIN")
        btn_all_min.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_BLUE}; padding: 6px; border-radius: 4px; font-size: 11px; }} QPushButton:hover {{ background-color: #47A1FF; }}")
        btn_all_min.clicked.connect(self.preset_all_min)
        
        btn_surge = QPushButton("SURGE")
        btn_surge.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_BLUE}; padding: 6px; border-radius: 4px; font-size: 11px; }} QPushButton:hover {{ background-color: #47A1FF; }}")
        btn_surge.clicked.connect(self.preset_surge)

        btn_all_max = QPushButton("MAX")
        btn_all_max.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_BLUE}; padding: 6px; border-radius: 4px; font-size: 11px; }} QPushButton:hover {{ background-color: #47A1FF; }}")
        btn_all_max.clicked.connect(self.preset_all_max)
        preset_row = create_row("QUICK", [btn_all_min, btn_surge, btn_all_max])
        
        btn_all_stop = QPushButton("ALL STOP")
        btn_all_stop.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Minimum)
        btn_all_stop.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_RED}; padding: 8px; border-radius: 4px; font-size: 13px; font-weight: bold;}} QPushButton:hover {{ background-color: #FF6961; }}")
        stop_row = create_row("DANGER", [btn_all_stop], stretch=False) 
        ...
        tab_auto_layout.setContentsMargins(0, 0, 0, 0)
        
        scroll_auto = QScrollArea()
        scroll_auto.setWidgetResizable(True)
        scroll_auto.setStyleSheet("QScrollArea { border: none; background-color: transparent; }")
        
        content_auto = QWidget()
        content_auto.setStyleSheet("background-color: transparent;")
        auto_layout = QVBoxLayout(content_auto)
        auto_layout.setSpacing(10)
        auto_layout.setAlignment(Qt.AlignTop)

        self.btn_auto = QPushButton("AUTO: STATION KEEPING")
        self.btn_auto.setCheckable(True)
        self.btn_auto.setStyleSheet(f"QPushButton {{ background-color: #3A3A3C; color: {TEXT_WHITE}; padding: 8px; border-radius: 14px; font-size: 13px; font-weight: bold; }} QPushButton:hover {{ background-color: #646468; }}")
        ...
        def create_target_row(name, key, min_val, max_val, step):
            row = QHBoxLayout()
            lbl = QLabel(name)
            lbl.setStyleSheet(f"color: {TEXT_WHITE}; font-size: 13px; font-weight: bold;")
            
            sp = QDoubleSpinBox()
            sp.setRange(min_val, max_val)
            sp.setSingleStep(step)
            sp.setDecimals(2)
            sp.setStyleSheet(f"background-color: #2C2C2E; color: {TEXT_WHITE}; border: 1px solid #444; border-radius: 4px; padding: 4px; font-size: 13px;")
            self.sp_spinboxes[key] = sp
            
            btn = QPushButton("SET")
            btn.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_BLUE}; padding: 4px 10px; border-radius: 4px; font-size: 11px; font-weight: bold; }} QPushButton:hover {{ background-color: #47A1FF; }}")
            btn.clicked.connect(lambda _, k=key, s=sp: self.set_individual_target(k, s.value()))

            curr_lbl = QLabel("Cur: 0.00")
            curr_lbl.setStyleSheet(f"color: {TEXT_DIM}; font-size: 13px; font-weight: bold;")
            curr_lbl.setAlignment(Qt.AlignRight | Qt.AlignVCenter)
            self.auto_current_labels[key] = curr_lbl 
        ...
        auto_layout.addLayout(create_target_row("Depth:", 'depth', -100.0, 100.0, 0.1))
        auto_layout.addLayout(create_target_row("Roll:", 'roll', -180.0, 180.0, 1.0))
        auto_layout.addLayout(create_target_row("Pitch:", 'pitch', -180.0, 180.0, 1.0))
        auto_layout.addLayout(create_target_row("Yaw:", 'yaw', -180.0, 180.0, 1.0))

        scroll_auto.setWidget(content_auto)
        tab_auto_layout.addWidget(scroll_auto)
        ...
        scroll_gain.setStyleSheet("QScrollArea { border: none; background-color: transparent; }")
        ...
        lbl_gain = QLabel("SDRE GAINS (COMMA SEPARATED)")
        lbl_gain.setStyleSheet(f"color: {ACCENT_ORANGE}; font-size: 13px; font-weight: bold;")
        ...
        def create_gain_row(name, key, default_val):
            ...
            inp.setStyleSheet(f"background-color: #2C2C2E; color: {TEXT_WHITE}; border: 1px solid #444; border-radius: 4px; padding: 4px; font-family: monospace; font-size: 12px;")
        ...
        self.btn_update_gain = QPushButton("UPDATE")
        self.btn_update_gain.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_ORANGE}; color: black; padding: 6px; border-radius: 4px; font-size: 12px; font-weight: bold; }} QPushButton:hover {{ background-color: #ffad33; }}")
        self.btn_update_gain.clicked.connect(self.update_controller_gains)
        btn_gain_row.addWidget(self.btn_update_gain)

        self.btn_save_gain = QPushButton("💾 SAVE")
        self.btn_save_gain.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_GREEN}; color: black; padding: 6px; border-radius: 4px; font-size: 12px; font-weight: bold; }} QPushButton:hover {{ background-color: #3BEA55; }}")
        ...
        def create_dpad_btn(text, surge=0, sway=0, heave=0, yaw=0, roll=0, pitch=0):
            ...
            # 🟢 ปลดล็อกการตั้งค่าขนาดตายตัว ให้ปุ่มบีบและยืดได้ตามหน้าจอ
            btn.setMinimumSize(35, 35) 
            btn.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
            btn.setStyleSheet(f"QPushButton {{ background-color: #3A3A3C; border-radius: 8px; font-weight: bold; font-size: 11px; }} QPushButton:hover {{ background-color: #646468; }} QPushButton:pressed {{ background-color: {ACCENT_GREEN}; color: white; }}")
        ...
        for i in range(dpad_grid.count()):
            ...
            if isinstance(w, QLabel) and w.text() in ["XY", "Z", "YAW", "PTC", "ROL"]:
                w.setStyleSheet(f"color: {TEXT_DIM}; font-size: 14px; font-weight: bold;") 
        ...
        self.btn_tare_roll.setStyleSheet("""QPushButton { background-color: #3A3A3C; padding: 4px; font-size: 11px; border-radius: 4px; } QPushButton:hover { background-color: #505052; }""")
        ...
        self.btn_tare_pitch.setStyleSheet("""QPushButton { background-color: #3A3A3C; padding: 4px; font-size: 11px; border-radius: 4px; } QPushButton:hover { background-color: #505052; }""")
        ...
        self.btn_tare_yaw.setStyleSheet("""QPushButton { background-color: #3A3A3C; padding: 4px; font-size: 11px; border-radius: 4px; } QPushButton:hover { background-color: #505052; }""")
        ...
        self.btn_calib.setStyleSheet("""QPushButton { background-color: #3A3A3C; padding: 4px; font-size: 11px; border-radius: 4px; } QPushButton:hover { background-color: #505052; }""")
        ...
        self.plot_lin, self.line_lx, self.line_ly, self.line_lz = self.create_plot("Linear Acceleration (ẍ, ÿ, z̈)", ["ẍ", "ÿ", "z̈"])
        ...
        # 🟢 ปล่อยให้กราฟขยายเต็มพื้นที่ที่เหลือ
        self.left_layout.addWidget(graphs_card, stretch=6)
        self.left_layout.addWidget(file_status_card)

        right_layout.addWidget(mission_card)
        right_layout.addLayout(self.row2_layout)
        right_layout.addWidget(sig_card)
        right_layout.addWidget(ctrl_card)
        right_layout.addWidget(osd_card)
    ...
    def toggle_fullscreen_cam(self):
        if not self.is_fullscreen_mode:
            ...
            self.fs_batt_layout.addWidget(self.hull_card)
            self.fs_batt_layout.addWidget(self.thrust_batt_card)
            ...
            self.btn_full.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_RED}; color: white; padding: 5px 15px; border-radius: 4px; font-size: 11px; }}")
            self.is_fullscreen_mode = True
        else:
            self.left_layout.insertWidget(0, self.cam_card, stretch=5)
            self.row2_layout.insertWidget(0, self.hull_card, stretch=3)
            self.row2_layout.insertWidget(1, self.thrust_batt_card, stretch=3)
    ...
    def toggle_auto_mode(self):
        ...
        if self.is_auto_mode:
            ...
            self.btn_auto.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_ORANGE}; color: #121212; padding: 10px 12px; border-radius: 14px; font-size: 14px; font-weight: bold; }}")
            ...
            self.btn_auto.setText("AUTO: STATION KEEPING")
            self.btn_auto.setStyleSheet(f"QPushButton {{ background-color: #3A3A3C; color: {TEXT_WHITE}; padding: 10px 12px; border-radius: 14px; font-size: 14px; font-weight: bold; }} QPushButton:hover {{ background-color: #646468; }}")
    ...
    def create_plot(self, title, labels):
        ...
        # 🟢 ลบการฟิกซ์ความสูงกราฟ ให้หดตัวได้อิสระ
...
def main(args=None):
    ...
    # 🟢 บังคับสั่งโชว์แบบเต็มจอ โดยไม่ใช้ QTimer หน่วงเวลา
