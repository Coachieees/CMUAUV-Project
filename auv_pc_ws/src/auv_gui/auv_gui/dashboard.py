...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                             ...
                             QTabWidget, QDoubleSpinBox, QStackedWidget, QLineEdit, QScrollArea) # 🟢 เพิ่ม QScrollArea
...
class ROS2Thread(QThread):
    ...
    def run(self):
        ...
        self.pub_gain = self.node.create_publisher(String, '/auv/tune_gains', 10) # 🟢 Topic สำหรับ Gain
...
class ModernDashboard(QMainWindow):
    def __init__(self):
        ...
        # 🟢 โหลดค่า Gain ล่าสุดที่บันทึกไว้ (ถ้ามี)
    ...
    def init_ui(self):
        ...
        self.combo_res.setStyleSheet(f"QComboBox {{ background-color: #2C2C2E; color: {TEXT_WHITE}; border-radius: 4px; padding: 5px; font-size: 12px; border: 1px solid #333; }} QComboBox::drop-down {{ border: none; }}")
        ...
        self.btn_restart.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_BLUE}; color: white; }} QPushButton:hover {{ background-color: #47A1FF; }}")
        ...
        self.tabs.setStyleSheet(f"QTabWidget::pane {{ border: 1px solid #333; border-radius: 4px; background: {BG_CARD}; top: -1px; }} QTabBar::tab {{ background: #2C2C2E; color: #8E8E93; padding: 8px 15px; border-top-left-radius: 4px; border-top-right-radius: 4px; margin-right: 2px; font-weight: bold; font-size: 13px; }} QTabBar::tab:selected {{ background: {ACCENT_BLUE}; color: white; }}")
        ...
        def create_row(title, widgets, stretch=True):
            ...
            lbl.setStyleSheet(f"color: {TEXT_DIM}; font-size: 13px; font-weight: bold;")
            l.addWidget(lbl)
        ...
        self.lbl_joy_status.setStyleSheet(f"color: {TEXT_DIM}; font-size: 14px; font-weight: bold;")
        
        self.btn_manual = QPushButton("🎮 MANUAL: OFF")
        self.btn_manual.setCheckable(True)
        self.btn_manual.setStyleSheet(f"QPushButton {{ background-color: #3A3A3C; padding: 8px 12px; border-radius: 18px; font-size: 13px; font-weight: bold; }} QPushButton:hover {{ background-color: #646468; }}")
        ...
        self.btn_ramp.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_ORANGE}; color: black; padding: 8px 12px; border-radius: 18px; font-size: 13px; font-weight: bold; }} QPushButton:hover {{ background-color: #ffad33; }}")
        self.btn_ramp.clicked.connect(self.toggle_ramp_mode)
        
        self.lbl_ramp_val = QLabel("0.25")
        self.lbl_ramp_val.setStyleSheet("background-color: #2C2C2E; color: #F5F5F7; padding: -6px; border-radius: 4px; font-weight: bold; font-family: monospace; font-size: 13px;")
        ...
        self.slider_ramp.setStyleSheet(f"QSlider::groove:horizontal {{ border-radius: 3px; height: 6px; background: #2C2C2E; }} QSlider::handle:horizontal {{ background: {ACCENT_ORANGE}; width: 20px; height: 22px; margin: -3px 0; border-radius: 5px; }}")
        ...
        btn_all_stop.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_RED}; padding: 8px 20px; border-radius: 6px; font-size: 14px; font-weight: bold;}} QPushButton:hover {{ background-color: #FF6961; }}")
        ...
        scroll_auto.setStyleSheet("QScrollArea { border: none; background-color: transparent; } QScrollBar:vertical { background: #1C1C1E; width: 12px; } QScrollBar::handle:vertical { background: #3A3A3C; border-radius: 6px; }")
        ...
        def create_target_row(name, key, min_val, max_val, step):
            row = QHBoxLayout()
            lbl = QLabel(name)
            lbl.setStyleSheet(f"color: {TEXT_WHITE}; font-size: 14px; font-weight: bold;")
            ...
            sp.setDecimals(2)
            sp.setStyleSheet(f"background-color: #2C2C2E; color: {TEXT_WHITE}; border: 1px solid #444; border-radius: 4px; padding: 6px; font-size: 14px;")
            sp.setFixedHeight(34)
            self.sp_spinboxes[key] = sp
            
            btn = QPushButton("SET")
            btn.setStyleSheet(f"QPushButton {{ background-color: {ACCENT_BLUE}; padding: 6px 12px; border-radius: 4px; font-size: 13px; font-weight: bold; }} QPushButton:hover {{ background-color: #47A1FF; }}")
            btn.clicked.connect(lambda _, k=key, s=sp: self.set_individual_target(k, s.value()))
            btn.setFixedHeight(34)

            curr_lbl = QLabel("Curent: 0.00")
            curr_lbl.setStyleSheet(f"color: {TEXT_DIM}; font-size: 14px; font-weight: bold;")
        ...
        auto_layout.addLayout(create_target_row("Depth(m):", 'depth', -100.0, 100.0, 0.1))
        ...
        # --- 🟢 Tab 3: Gain Config (ใส่ Scroll + เพิ่ม Font ให้ใหญ่ขึ้น) ---
        tab_gain = QWidget()
        ...
        lbl_gain.setStyleSheet(f"color: {ACCENT_ORANGE}; font-size: 14px; font-weight: bold;")
        ...
        def create_gain_row(name, key, default_val):
            ...
            inp.setStyleSheet(f"background-color: #2C2C2E; color: {TEXT_WHITE}; border: 1px solid #444; border-radius: 4px; padding: 6px; font-family: monospace; font-size: 13px;")
        ...
        # 🟢 สร้าง Layout แนวนอนสำหรับวางปุ่มคู่กัน
        ...
        # 🟢 เพิ่มทั้ง 3 Tab เข้าไปในหน้าจอ
